		...
		<view>
			<uni-section type="line" padding>
				...
				<view class="studentnumtwo">
					<uni-easyinput class="uni-mt-5" trim="all" v-model="date.Sid" placeholder="请输入学号">
					</uni-easyinput>
				</view>
			</uni-section>
		</view>
		<view>
			<button @click="binding">登录</button>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
						...
						uni.getUserInfo({
							...
							success: function(res) {
								uni.login({
									success: res => {
										uni.request({
											url: 'https://api.weixin.qq.com/sns/jscode2session?appid=wx85b852e5aab616da&secret=863f2f470108f6c82c546488d64f1942&js_code=' +
												res.code +
		...
		methods: {
				...
				uni.getUserProfile({
					...
					success: res => {
						...
						uni.login({
							success: res => {
								uni.request({
									url: 'https://api.weixin.qq.com/sns/jscode2session?appid=wxfe85c92a33a8fb07&secret=863f2f470108f6c82c546488d64f1942&js_code=' +
										res.code +
										'&grant_type=authorization_code',
									success: res => {

										this.date.WeChatOpenId = res.data.openid
										that.isHide = false
			...
			binding: function() {
				var that = this;
				console.log(that.date.Sid);
				uni.request({
					url: `http://localhost:5038/bind`,
					method: "POST",
					data: {
						WeChatOpenId: that.date.WeChatOpenId,
						StudentCode: that.date.Sid
					},
					success: res => {
						console.log(res);
						if (res.data.code == 4000) {
							...
							var userinfo = {
								photo: that.date.AvatarUrl,
								nickName: that.date.UserName,
								data: res.data
							}
